...
<body class="bg-gray-50 text-gray-800 font-sans antialiased min-h-screen flex">
            ...
            @auth
                @if(auth()->user()->role == 'admin')
                    ...
                    <a href="{{ route('school.dashboard') }}" class="nav-item {{ request()->routeIs('school.dashboard') ? 'active' : '' }}">
                        ...
                        <span class="label">Dashboard Sekolah</span>
                    </a>
                    ...
                    <a href="{{ route('gallery.manage') }}" class="nav-item {{ request()->routeIs('gallery.manage') ? 'active' : '' }}">
                        <span class="icon"><i class="ph ph-images"></i></span>
                        <span class="label">Kelola Gallery</span>
                    </a>
                    
                    <div class="nav-divider"></div>
                    
                    <a href="{{ route('absensi.create') }}" class="nav-item {{ request()->routeIs('absensi.create') ? 'active' : '' }}">
                        <span class="icon"><i class="ph ph-calendar-check"></i></span>
                        <span class="label">Input Absensi</span>
                        <span class="badge">New</span>
                    </a>
                    
                    <a href="{{ route('absensi.index') }}" class="nav-item {{ request()->routeIs('absensi.index') ? 'active' : '' }}">
                        <span class="icon"><i class="ph ph-clipboard-text"></i></span>
                        <span class="label">Daftar Absensi</span>
                    </a>
                @else
                    <a href="{{ route('dashboard') }}" class="nav-item {{ request()->routeIs('dashboard') ? 'active' : '' }}">
                        <span class="icon"><i class="ph ph-gauge"></i></span>
                        <span class="label">Dashboard Siswa</span>
                    </a>
                ...
                <div class="nav-item" onclick="toggleTheme()" style="cursor: pointer;">
                    <span class="icon"><i class="ph ph-sun-dim" id="sidebar-theme-icon"></i></span>
                    <span class="label" id="sidebar-theme-label">Toggle Theme</span>
                </div>
            @endauth
        </div>
            ...
            @auth
                <div class="user-info">
                    ...
                    <div>
                        ...
                        <div class="user-role">{{ auth()->user()->role == 'admin' ? 'Administrator' : auth()->user()->kelas ?? 'Siswa' }}</div>
                    </div>
                </div>
                
                <form action="{{ route('logout') }}" method="POST" class="mt-3">
                    @csrf
                    <button type="submit" class="nav-item w-full" style="background: rgba(239, 68, 68, 0.1); color: #f87171; border: none; width: 100%; text-align: left;">
    ...
    <!-- Toggle Button - Arrow in the middle -->
    <button id="navToggle" class="nav-toggle" onclick="toggleNav()" aria-label="Toggle Navigation">
    ...
    <main id="mainContent" class="main-content expanded">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            @if(session('success'))
                <div class="mb-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 px-4 py-3 rounded-lg flex items-center gap-3 shadow-sm">
            ...
            @if(session('error'))
                <div class="mb-4 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 px-4 py-3 rounded-lg flex items-center gap-3 shadow-sm">
            ...
            @if(session('warning'))
                <div class="mb-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 px-4 py-3 rounded-lg flex items-center gap-3 shadow-sm">
    ...
    <div id="toast" class="fixed bottom-4 right-4 bg-gray-800 text-white px-4 py-3 rounded-lg shadow-lg transform translate-y-20 opacity-0 transition-all duration-300 flex items-center gap-3 z-50">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const savedState = localStorage.getItem('navOpen');
            const arrow = document.getElementById('navArrow');
        ...
        // Also sync with main theme toggle
        document.addEventListener('DOMContentLoaded', function() {
